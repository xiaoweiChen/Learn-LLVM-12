后端的任务是从模块的IR中创建优化的机器码。IR是后端的接口，可以使用C++接口或文本形式创建。同样，IR是由AST生成的。\par
...
我们实现了编译器的前端，从读取源代码到生成IR。当然，所有这些组件必须在用户输入时协同工作，这是编译器驱动程序的任务，我们还需要实现运行时所需的功能。我们将在下一节中讨论这两个问题。\par
