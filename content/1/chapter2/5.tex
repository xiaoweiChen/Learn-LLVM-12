...
Ubuntu和Debian发行版都有支持交叉编译的软件包。在下面的设置中，我们将利用这一点。gcc和g++编译器、ld链接器和库都可以作为生成ARMv8代码和可执行文件的预编译二进制文件使用。要安装所有这些包，输入以下命令:\par

\begin{tcolorbox}[colback=white,colframe=black]
	\$ sudo apt install gcc-8-aarch64-linux-gnu $\setminus$ \\
	g++-8-aarch64-linux-gnu binutils-aarch64-linux-gnu $\setminus$ \\
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black, title=Cross-compiling on other systems]
如果您的发行版没有附带所需的工具链，那么您可以从源代码构建它。必须配置gcc和g++编译器来生成目标系统的代码，binutils工具需要处理目标系统的文件。此外，C和C++库需要用这个工具链来编译。该步骤因所使用的操作系统以及主机和目标体系结构而异。可以通过网页上，搜索gcc cross-compile <architecture>，从而找到找到相应的指令。
\end{tcolorbox}

通过这些准备，您几乎已经准备好交叉编译示例应用程序(包括LLVM库)了，除了一个小细节外。LLVM在构建过程中使用tablegen工具。交叉编译期间，为目标体系结构编译所有内容，包括此工具。可以在第1章“安装LLVM”中使用llvm-tblgen，也可以只编译这个工具。假设你在包含GitHub存储库克隆的目录下，输入如下:\par

\begin{tcolorbox}[colback=white,colframe=black]
	\$ mkdir build-host \\
	\$ cd build-host \\
	\$ cmake -G Ninja $\setminus$ \\
	-DLLVM\underline{~}TARGETS\underline{~}TO\underline{~}BUILD="X86" $\setminus$ \\
	-DLLVM\underline{~}ENABLE\underline{~}ASSERTIONS=ON $\setminus$ \\
	-DCMAKE\underline{~}BUILD\underline{~}TYPE=Release $\setminus$ \\
	../llvm-project/llvm \\
	\$ ninja llvm-tblgen \\
	\$ cd ..
\end{tcolorbox}
